...
An agent's authority over the fleet is one command: `swarm stop`, on an instance
it spawned itself. That is how a parent takes back work it handed out, and it
collects the instance rather than merely stopping it, since one that came back
would have no memory of the task it still owes. A peer, a declared agent or
somebody else's instance is refused.

`swarm start`, `swarm restart` and `swarm shutdown` are refused to every agent.
The first two are not a rule so much as an absence of use: a live instance is
already running, a stopped one has been collected and is gone from the fleet,
and restarting an instance is refused whoever asks. Ending the fleet is not work
at all.
