...
# trimmed. max: 0 switches the memory off.
#
# A full memory drops the entry the fleet has gone longest without writing or
# looking up, and ttl drops one nobody has wanted in that long — both measure
# use rather than age, because the oldest entry is often the one everything
# rests on. ttl: 0s keeps entries forever. Every change is appended to
# .swarm/memory.log, including the line that was written over.
#
# memory:
#   max: 50
#   chars: 200
